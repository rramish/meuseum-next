...
//   const { setEraser, eraser } = useToolsStore();
...
const Sidebar = () => {
  ...
  const setEraserTool = () => {
    const canvas = canvasRef?.current as unknown as fabric.Canvas;
    if (canvas) {
      canvas.isDrawingMode = true; // Enable drawing mode for eraser
      const eraserBrush = new fabric.PencilBrush(canvas);
      eraserBrush.width = brushSize;
      eraserBrush.color = "#FFFFFF"; // Match canvas background (adjust if different)
  ...
  // Remove the useEffect since it relies on getActiveObjects
  // useEffect(() => {
  ...
  //       canvas.discardActiveObject();
  //       canvas.renderAll();
  //       console.log("Removed active objects");
  //     }
  //   }
  // }, [eraser, canvasRef]);
